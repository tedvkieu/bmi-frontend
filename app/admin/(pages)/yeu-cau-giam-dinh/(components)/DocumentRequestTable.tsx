...
const DocumentRequestTable: React.FC<DocumentRequestTableProps> = ({
    ...
    const [filterType, setFilterType] = useState<"all" | CustomerType | "IMPORTER" | "EXPORTER">("all");
    ...
    const filteredAndSortedRequests = useMemo(() => {
        ...
        // 1. Filter
        if (filterType !== "all") {
            tempRequests = tempRequests.filter(req => req.customerType === filterType);
        }
    ...
    const getSortIcon = (key: SortKey) => {
        ...
        return sortOrder === "asc" ? <ArrowUpAZ size={14} /> : <ArrowDownAZ size={14} />;
    };
                                ...
                                <table className="min-w-full divide-y divide-gray-200">
                                    <thead>
                                        <tr>
                                            ...
                                            <th className="px-6 py-4 text-left text-xs font-semibold text-gray-800 uppercase">Số điện thoại</th>
                                            <th className="px-6 py-4 text-left text-xs font-semibold text-gray-800 uppercase">Loại khách hàng</th>
                                            <th
                                                className="px-6 py-4 text-left text-xs font-semibold text-gray-800 uppercase cursor-pointer hover:bg-gray-50"
                                    ...
                                    <tbody className="bg-white divide-y divide-gray-100">
                                        {currentItems.length === 0 ? (
                                            ...
                                            currentItems.map((request) => (
                                                <tr key={request.customerId} className="hover:bg-blue-50 transition-colors duration-200">
                                                    <td className="px-6 py-4 text-sm text-gray-900 max-w-[180px] truncate" title={request.name}>
                                                        {request.name}
                                                    </td>
                                                    ...
                                                    <td className="px-6 py-4 text-sm">
                                                        <div className="flex justify-center gap-2">
                                                            <button
                                                                onClick={() => handleClickPage(request.customerId)}
