...
export const userApi = {
  ...
  async getDossiersByUser(
    userId: number,
    page = 0,
    size = 10,
    opts?: { q?: string; month?: number; year?: number }
    ...
    if (typeof opts?.month === "number" && opts.month >= 1 && opts.month <= 12)
      params.set("month", String(opts.month));
    if (typeof opts?.year === "number" && opts.year > 0)
      params.set("year", String(opts.year));
    const res = await fetch(
      `/api/users/${userId}/dossiers?${params.toString()}`,
      {
        headers: authHeaders(),
        cache: "no-store",
      }
