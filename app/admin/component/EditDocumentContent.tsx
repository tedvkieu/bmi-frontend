...
const EditDocumentContent: React.FC<EditDocumentPageProps> = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setError(null);
        try {
            await axios.put(`/api/receipts/${id}`, document); 
    ...
    const getFieldName = (label: string): keyof InspectionReportApi => {
        switch (label) {
            case "Mã hồ sơ": return "receiptId";
            case "Số đăng ký": return "registrationNo";
            case "Khách hàng yêu cầu giám định": return "customerSubmitId";
            case "Khách hàng nhập khẩu": return "customerRelatedId";
            case "Loại hình giám định": return "inspectionTypeId";
            case "Số tờ khai": return "declarationNo";
            case "Số vận đơn": return "billOfLading"; // Fixed: Added return
            case "Tên tàu": return "shipName";
            case "Số lượng Container 20 Feets": return "cout20";
            case "Số lượng Container 40 Feets": return "cout10"; // Assuming cout10 is for 40 feets
            case "Trạng thái rời cảng": return "bulkShip";
            case "Tài liệu khai báo": return "declarationDoc";
            case "Nơi khai báo": return "declarationPlace";
            case "Ngày kiểm tra": return "inspectionDate";
            case "Ngày cấp chứng chỉ": return "certificateDate";
            case "Địa điểm kiểm tra": return "inspectionLocation";
            case "Trạng thái chứng chỉ": return "certificateStatus";
            case "Ngày tạo hồ sơ": return "createdAt";
            default: return "receiptId"; // Fallback, adjust as needed
        }
    };
            ...
            <div className="col-span-1">
                <label htmlFor={fieldName} className="block text-sm font-medium text-gray-700 mb-1">
                    <div className="flex items-center space-x-2">
                        <Icon size={16} className="text-gray-500" /> {/* Smaller, more neutral icon color */}
                        <span>{label}</span>
                        <span className="text-gray-500 text-sm hidden md:inline group-hover:block transition-all duration-300">({tooltip})</span>
                    </div>
                </label>
                {inputField()}
            </div>
    ...
    const renderDateItem = (
        ...
        const fieldName = getFieldName(label);
        const dateValue = dateString ? new Date(dateString).toISOString().split('T')[0] : "";
        return (
            <div className="col-span-1">
                <label htmlFor={fieldName} className="block text-sm font-medium text-gray-700 mb-1">
                    <div className="flex items-center space-x-2">
                        ...
                        <span>{label}</span>
                        <span className="text-gray-500 text-xs hidden md:inline group-hover:block transition-all duration-300">({tooltip})</span>
                    </div>
                </label>
                <input
                    type="date"
    ...
    const renderDateTimeItem = (
        ...
        const fieldName = getFieldName(label);
        const dateTimeValue = dateString ? new Date(dateString).toISOString().slice(0, 16) : ""; // For datetime-local input
        return (
            <div className="col-span-1">
                <label htmlFor={fieldName} className="block text-sm font-medium text-gray-700 mb-1">
                    <div className="flex items-center space-x-2">
    ...
    return (
        <div className="container mx-auto  text-gray-800">
            <div className="bg-white rounded-xl shadow-lg w-full max-w-7xl mx-auto flex flex-col border border-gray-200"> {/* Softer shadow */}
                {/* Header - Simplified */}
                <div className="flex justify-between items-center p-4 border-b border-gray-200 bg-gray-50 rounded-t-xl">
                    <h2 className="text-xl font-semibold flex items-center gap-2 text-gray-800">
                        ...
                        <span className="font-bold text-blue-700">{document.registrationNo || document.billOfLading || id}</span> {/* Keep identifier highlighted */}
                    </h2>
                </div>

                <form onSubmit={handleSubmit} className="flex-grow overflow-y-auto p-7 space-y-6 bg-white"> {/* White background for content */}
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-x-8 gap-y-5"> {/* Slightly reduced gaps */}
                        {/* Dynamically render editable fields */}
                        {renderDetailItem("Mã hồ sơ", document.receiptId, "Receipt ID", Tag, false)} {/* Đã sửa thành readonly */}
                        {renderDetailItem("Số đăng ký", document.registrationNo, "Số đăng ký của tài liệu hoặc phương tiện", Hash, false)} {/* Đã sửa thành readonly */}
                        ...
                        {renderDetailItem("Trạng thái chứng chỉ", document.certificateStatus, "Certificate Status", CheckCircle)}
                        {renderDateTimeItem("Ngày tạo hồ sơ", document.createdAt, "Created At", Clock, false)} {/* Read-only */}
                    </div>

                    {/* Footer buttons - Simplified */}
                    <div className="p-4 border-t border-gray-200 bg-gray-50 flex justify-end space-x-3 rounded-b-xl mt-6"> {/* Light gray footer, slightly rounded */}
                        <button
                            type="button"
