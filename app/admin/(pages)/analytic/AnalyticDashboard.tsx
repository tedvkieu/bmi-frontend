        ...
        return (
            <div className="bg-white p-3 border border-gray-200 rounded-lg shadow-md text-sm">
                ...
                <p className="font-semibold text-gray-800 mt-1">
                    Tổng cộng: <span className="font-medium">
                        {payload.reduce((sum: number, entry: any) => sum + entry.value, 0).toLocaleString('vi-VN')}
                    </span>
                </p>
            </div>
        );
    ...
    const fetchAnalyticData = useCallback(async () => {
        try {
            ...
            if (selectedMonth === 0) {
                ...
                if (!statusByMonthRes.ok)
                    throw new Error(`Lỗi HTTP khi tải dữ liệu trạng thái theo tháng: ${statusByMonthRes.status}`);
